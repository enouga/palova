...
export function MemberProfileCard({ member, onSave, error }: {
  ...
  const label: CSSProperties = { fontFamily: th.fontUI, fontSize: 11, fontWeight: 700, color: th.textMute, textTransform: 'uppercase', letterSpacing: 0.3, display: 'block', margin: '0 0 4px' };
  ...
  return (
    <section aria-label="Profil" style={memberCardStyle(th)}>
      <Kicker color={MEMBER_CARD_TINTS.blue}>Profil</Kicker>
      {error && <div style={{ marginBottom: 8, fontFamily: th.fontUI, fontSize: 12.5, color: th.danger }}>{error}</div>}
      {/* Champs en grille 2 colonnes (adresse et note en pleine largeur) : le formulaire
          tient deux fois moins haut que l'ancienne pile verticale. */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '9px 8px' }}>
        ...
        <div><span style={label}>Code postal</span><input aria-label="Code postal" value={draft.postalCode} onChange={(e) => set('postalCode', e.target.value)} style={input} /></div>
        <div><span style={label}>Ville</span><input aria-label="Ville" value={draft.city} onChange={(e) => set('city', e.target.value)} style={input} /></div>
        <div>
        ...
        <div><span style={label}>N° licence / adhérent</span><input aria-label="N° licence / adhérent" value={draft.membershipNo} onChange={(e) => set('membershipNo', e.target.value)} style={input} /></div>
        <div style={full}><span style={label}>Note</span><textarea aria-label="Note" value={draft.note} onChange={(e) => set('note', e.target.value)} rows={2} style={{ ...input, resize: 'vertical' }} /></div>
      </div>
