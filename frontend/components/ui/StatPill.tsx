...
export function StatPill({ icon, accent, label, value, meter, warn, fill }: StatPillProps) {
  const { th } = useTheme();
  ...
  return (
    <span
      ...
      style={{
        display: fill ? 'flex' : 'inline-flex', width: fill ? '100%' : undefined,
      ...
      <span style={{ display: 'flex', flexDirection: 'column', gap: meter ? 4 : 1, minWidth: 0 }}>
        ...
        {meter ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ width: 50, height: 5, borderRadius: 999, background: th.surface2, overflow: 'hidden' }}>
            ...
            <span style={{ fontFamily: th.fontUI, fontSize: 10.5, fontWeight: 500, color: th.textFaint }}>{meter.suffix}</span>
          </span>
        ) : (
          <span style={{ fontFamily: th.fontUI, fontSize: 15, fontWeight: 750, color: th.text, letterSpacing: -0.1, fontVariantNumeric: 'tabular-nums' }}>
            {value}
          </span>
        )}
      </span>
