...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const sectionHeaderStyle = {
    ...
    textTransform: 'uppercase' as const, color: th.textFaint, padding: 'clamp(1px, 1vh - 7px, 6px) 10px',
  };
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: th.bg, color: th.text, fontFamily: th.fontUI, display: 'flex' }}>
      ...
      <aside style={{
        ...
        display: 'flex', flexDirection: 'column', padding: 'clamp(8px, 2.4vh - 11.2px, 20px) 14px',
      }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 1, overflowY: 'auto', marginTop: 'clamp(2px, 1.6vh - 10.8px, 10px)' }}>
              ...
              <div key={sec.title ?? 'top'} role="group" aria-label={sec.title} style={{ display: 'contents' }}>
                {sec.title && (
                  <button type="button" onClick={() => toggleSection(sec.title!)} aria-expanded={!isCollapsed}
                    ...
                    style={{
                      ...sectionHeaderStyle, marginTop: i === 0 ? 0 : 'clamp(2px, 2vh - 14px, 12px)',
                ...
                {!isCollapsed && sec.items.map((l) => {
                  ...
                  return (
                    <Link key={l.href} href={l.href} style={{
                      display: 'flex', alignItems: 'center', gap: 11, padding: 'clamp(1px, 1.6vh - 11.8px, 9px) 12px', borderRadius: 11, textDecoration: 'none',
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 8, paddingTop: 'clamp(6px, 2vh - 10px, 16px)', borderTop: `1px solid ${th.line}` }}>
          <ThemeToggle />
